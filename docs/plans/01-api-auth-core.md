...
- Wire up REST controllers with Swagger/OpenAPI docs and decide how auth guards apply by default across controllers.
...
- JWT strategy: short-lived access token (e.g. 15 min, kept small so a leaked token has a tight blast radius) + **7-day refresh token**. `POST /auth/refresh` endpoint alongside `POST /auth/login`. **Refresh token is set as an `httpOnly`, `Secure`, `SameSite=Strict` cookie** on the response (not returned in the response body) — never reachable by frontend JS, mitigating XSS token theft; the access token is returned in the `login`/`refresh` response body for the frontend to hold in memory (per [Milestone 03](./03-web-app-shell.md)).
- REST controllers (`@nestjs/swagger` decorators for OpenAPI docs), configured for cookie parsing (`cookie-parser`) and CORS `credentials: true` restricted to the app's known origin(s) (dev: `localhost`; prod: the app's real origin — never a wildcard, required for cookies to work cross-origin at all)
- `JwtAuthGuard` applied by default to controllers except `login`/`refresh`
...
- `POST /auth/login` returns an access token in the response body and sets the refresh-token cookie (`httpOnly`, not visible to `document.cookie` in a browser console).
- `POST /auth/refresh` reads the refresh cookie, returns a new access token; fails on an expired/invalid/missing cookie.
- A guarded endpoint fails without a token and succeeds with one.
- Changing `INITIAL_USERNAME`/`INITIAL_PASSWORD` and restarting rotates the seeded user's credentials.
- Swagger UI reachable in dev from the Vite dev server origin (CORS + cookie working cross-port).
