# Sub-Plan 12 — Ticker universe / XTB availability
...
- **Credentials:** stored via `SecretsService` ([Sub-Plan 02](./02-secrets-management.md)), managed from a new **Settings → Brokers tab** (confirmed with user) — add/edit/remove broker credentials (e.g. XTB login), same pattern as `ChannelConfig` in [Sub-Plan 07](./07-notification-channels.md). This is the first broker; the tab is built generically enough to list multiple broker types later.
- **Active broker (confirmed with user):** a global setting (single value, e.g. on the user/app settings document) selects which configured broker's data is used platform-wide to drive the "tradable" badge/filter on `WatchlistWidget` ([Sub-Plan 06](./06-watchlist-tab.md)) and `MoversWidget` ([Sub-Plan 05](./05-market-data.md)). The existing per-widget filter toggle (default off) still governs whether the filter is actually applied on each widget instance; the global setting just determines *which broker's* availability data feeds those badges/toggles.
