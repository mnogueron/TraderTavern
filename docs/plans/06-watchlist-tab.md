...
As a developer, I want to build full CRUD watchlists backed by live quotes, so that I practice a complete feature end-to-end — schema, resolvers, and UI — reusing shared building blocks rather than duplicating logic.
...
- **Symbol search:** a `searchSymbols(query)` resolver added to `IMarketDataProvider` ([Milestone 05](./05-market-data.md)) backing an autocomplete-as-you-type UI when adding a symbol to a list — matches against real ticker data (name/symbol) rather than free-text entry, reducing typos/invalid tickers.
